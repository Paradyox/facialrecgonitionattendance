"""
Clean Facial Recognition Attendance System
Classic face_recognition (dlib) pipeline with GUI and Excel attendance logs.
Python 3.13+
"""
...
class FacialRecognitionSystem:
    """Core recognition and attendance logic."""
    ...
    def _load_cache(self):
        """Load encoded faces from cache for fast startup."""
        try:
    ...
    def _dedupe_known_faces(self):
        """Keep one encoding per unique name (average duplicates if present)."""
    ...
    def _save_cache(self):
        """Persist encoded faces to cache file."""
        try:
    ...
    def load_known_faces(self):
        """Load faces from face_data/<person_name>/*.jpg and refresh cache."""
        self.known_face_encodings = []
        self.known_face_names = []
    ...
    def detect_and_recognize_faces(self, frame, model="hog", tolerance=0.6, device="cpu", far_mode=False):
        ...
        Note:
        - device is kept for UI compatibility.
        - face_recognition uses dlib underneath. GPU is only used if dlib was built with CUDA.
        """
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        # Keep classic behavior: model controls detector (hog/cnn).
        # device is advisory only.
    ...
    def register_face(self, person_name, image):
        """Register a new face image for a person."""
        try:
    ...
    def log_attendance(self, person_name, confidence=0.0):
        ...
        Update existing row only when new confidence is higher.
        ...
        try:
            ...
            # Push latest session state to dashboard JSON.
    ...
    def export_attendance_json(self, present_students=None):
        """Export current session attendance snapshot for classroom_dashboard.html."""
        try:
    ...
    def _format_attendance_sheet(self, excel_path):
        """Apply basic formatting to attendance spreadsheet."""
        try:
...
class AttendanceGUI:
    """Tkinter GUI wrapper for live attendance."""
    ...
    def apply_fast_preset(self):
        """Apply defaults optimized for quick classroom attendance."""
    ...
    def _list_available_cameras(self, max_index=8):
        """Probe camera indices and return available camera IDs."""
        found = []
    ...
    def refresh_camera_list(self):
        """Refresh camera drop-down with currently available devices."""
    ...
    def _selected_camera_index(self):
        """Return selected camera index from combo text."""
    ...
    def _cancel_auto_stop_timer(self):
        """Cancel scheduled auto-stop callback if present."""
    ...
    def _schedule_auto_stop_timer(self):
        """Schedule automatic camera stop according to UI settings."""
    ...
    def _auto_stop_camera(self):
        """Auto-stop callback for camera timeout."""
        self.auto_stop_after_id = None
    ...
    def _detect_faces_async(self, frame, scale):
        try:
            ...
            if self.far_mode_var.get() and effective_scale < 0.9:
                # Keep more detail when targeting distant faces.
                effective_scale = 0.9
    ...
    def update_frame(self):
        ...
        # Fit to available camera box size while preserving aspect ratio.
